---
aliases:
  - /items/show/26938
  - /items/show/26938.html
collections:
  - '103'
date: '2007-04-18 18:24:14'
draft: false
dublin_core:
  Description:
    - html: false
      text: Interview with Terry Davis from American Association of State and Local History who helped to organize a team to assess damage caused by Hurricane Katrina along the Gulf Coast.
  Language:
    - html: false
      text: eng
  Title:
    - html: false
      text: Terry Davis HK2556K.mp3
featured: false
files:
  - fullsize:
    id: 23115
    mime: audio/mpeg
    original_filename: Terry Davis HK2556K.mp3
    thumb:
    url: https://hurricanearchive.org/files/original/14fb29d601092314fb29d60116dd.mp3
geo:
id: 26938
item_type: Oral History
item_type_metadata: {}
lastmod: '2012-08-07 16:15:36'
layout: single
plugin_metadata: []
related_items: []
slug: '26938'
tags:
  - damage
  - historic preservation
  - museums
  - recovery
title: Terry Davis HK2556K.mp3
type: items
---

{{< raw >}}
Interview with Terry Davis from American Association of State and Local History who helped to organize a team to assess damage caused by Hurricane Katrina along the Gulf Coast.
{{< /raw >}}
