...
- Transcription panel with tabbed UI: human transcription (from `.Content`) and AI transcription (from `data/transcriptions_ai.json`, generated by the `claude -p` pipeline in `_transcription/` — in progress)
...
- AI transcriptions via `data/transcriptions_ai.json` with tabbed comparison UI (in progress — being generated by the `claude -p` pipeline in `_transcription/`; see `_transcription/README.md`)
