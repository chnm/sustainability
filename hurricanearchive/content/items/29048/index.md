---
aliases:
  - /items/show/29048
  - /items/show/29048.html
collections:
  - '162'
date: '2007-06-22 18:04:46'
draft: false
dublin_core:
  Description:
    - html: false
      text: Army Troops receive a Briefing prior to going on Katrina Relief missions.
  Language:
    - html: false
      text: eng
  Title:
    - html: false
      text: Base Buildup
featured: false
files:
  - fullsize: /files/fullsize/14fb29d06a3c0e14fb29d06a3ff9.jpg
    id: 23618
    mime: image/jpeg
    original_filename: LAANG-Photo-17.jpg
    thumb: /files/thumbnails/14fb29d06a3c0e14fb29d06a3ff9.jpg
    url: /files/original/14fb29d06a3c0e14fb29d06a3ff9.jpg
geo:
id: 29048
item_type: Still Image
item_type_metadata: {}
lastmod: '2008-01-02 17:37:37'
layout: single
plugin_metadata: []
related_items: []
slug: '29048'
tags:
  - 159th fighter wing
  - bellechasse nasjrb
title: Base Buildup
type: items
---

{{< raw >}}
Army Troops receive a Briefing prior to going on Katrina Relief missions.
{{< /raw >}}
