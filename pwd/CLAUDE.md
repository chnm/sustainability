...
Python scripts run under `uv` (`uv run python3 scripts/…`); tests live in `tests/` and run with `uv run pytest`. Data-munging scripts patch frontmatter with **minimal-diff regex string surgery** (never `yaml.dump`, which reorders/requotes keys across ~43k files) — follow that pattern.

## Architecture

### Data Flow

Content is pulled from the Omeka S REST API at `https://omeka.wardepartmentpapers.org/api/` (VPN-only; the old `www.wardepartmentpapers.org/api/` host is dead — 302→404):
...
Documents with transcriptions store the human-transcribed text in the markdown body (`.Content`). AI transcriptions are stored in `data/transcriptions_ai.json` keyed by `omeka_id` (Hugo reads this); the document template has a tabbed human/AI UI.

AI transcription is generated by **two** pipelines — the preferred `_transcription/transcribe.py` (`claude -p`, billed to a Claude **subscription**) and the alternate `scripts/transcribe.py` (Anthropic **SDK + API key**). The `claude -p` pipeline writes `_transcription/transcriptions.json`, which must be **manually synced** into `data/transcriptions_ai.json`. Runbook: `_transcription/README.md`.

### Document Images (microfilm reel model)

Documents reference a shared Omeka **Image resource** (microfilm reel) via `omeka_image_id`; `data/media_map.json` maps that reel to its ordered files. A document's `images:` list is a slice `reel_files[page_start-1 : page_start-1+num_pages]`. When `num_pages` is missing, docs were truncated to one image — `scripts/fix_multipage_images.py` repairs this **locally** (no API): single/small reels (≤5 imgs) get the whole reel, large shared reels are sliced by neighbor `page_start`. See `docs/superpowers/specs/2026-07-06-multipage-images-design.md`.
