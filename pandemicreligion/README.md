...
2. **Flatten** — `../scripts/pandemicreligion_flatten.py` copies the capture
   ...
   relativizes every `files/` reference to root-relative `/files/...`, replaces
   Omeka's Matomo double-tagging with a single per-site block (one siteId per
   site: 74 hub, 85 CTT + subsites, 86 PGV, 87 AJL, all on `stats.rrchnm.org`),
   rewires the header search form to a local Pagefind `search.html`,
   neutralizes dead submit forms, and tags real content for indexing.
